...
export default function AsadoCentral() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. Audio Activation on First Interaction
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          if (v && v.muted) {
            ...
            v.volume = 0;
            v.play().catch(() => {});
            gsap.to(v, { volume: 1, duration: 1.5 });
            setIsMuted(false);
          }
        },
        once: true
      ...
      // 2. Video & Audio Transition (FADE SMOOTHLY)
      // This maps volume to scroll position from 1 to 0 as we enter Experience section
      gsap.to(videoRef.current, {
        scrollTrigger: {
          trigger: '#experiencia',
          start: 'top bottom', // Start fading as Experience enters screen
          end: 'top 30%', // Reach 0 volume before it fully covers
          scrub: true,
          onUpdate: (self) => {
      ...
      // Change Video Source with a temporary fade-out to prevent "black flicker"
      ScrollTrigger.create({
        trigger: '#experiencia',
        start: 'top 60%',
        onEnter: () => {
      ...
      gsap.utils.toArray('.floating-item').forEach((el) => {
        gsap.to(el, { y: -30, scrollTrigger: { trigger: el, start: 'top bottom', end: 'bottom top', scrub: 1 } });
      });

      gsap.utils.toArray('.reveal-up').forEach((el) => {
        gsap.from(el, { y: 40, opacity: 0, duration: 0.8, ease: 'power2.out', scrollTrigger: { trigger: el, start: 'top 90%' } });
      });
    }, containerRef);
  ...
  return (
    <div ref={containerRef} className="text-white min-h-screen font-['Manrope'] overflow-x-hidden bg-black">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .global-overlay {
          ...
          background: linear-gradient(to bottom, rgba(13, 13, 13, 0.4) 0%, rgba(13, 13, 13, 1) 100%);
        }
        .glassmorphic-celeste-text {
          color: rgba(116, 172, 223, 0.15);
          -webkit-text-stroke: 1.5px rgba(116, 172, 223, 0.8);
          mix-blend-mode: color-dodge;
          text-shadow: 0 0 30px rgba(116, 172, 223, 0.4);
        }
        .sound-btn {
          position: fixed; bottom: 40px; right: 40px; z-index: 100;
          background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px);
          border: 1px solid rgba(255, 255, 255, 0.1); width: 60px; height: 60px;
          border-radius: 50%; display: flex; align-items: center; justify-content: center;
        }
        .wave-container { display: flex; align-items: center; gap: 2px; }
        .wave { width: 2px; height: 12px; background: #FF4D4D; animation: wave-anim 1s ease-in-out infinite alternate; }
        @keyframes wave-anim { 0% { height: 4px; } 100% { height: 16px; } }
      `}} />

      <div className="grain-overlay" />
      <video ref={videoRef} autoPlay muted loop playsInline className="global-bg" style={{ opacity: 1 }}>
      ...
      <button className="sound-btn" onClick={toggleSound}>
        {isMuted ? <VolumeX size={24} className="text-white/50" /> : (
          <div className="wave-container">
            {[0, 0.2, 0.4].map(d => <div key={d} className="wave" style={{animationDelay: `${d}s`}} />)}
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden hero-section z-10">
        <div className="relative z-20 text-center px-4 max-w-5xl">
          ...
          <a href="#selección" className="group relative inline-flex items-center justify-center pl-10 pr-2 py-2 overflow-hidden rounded-full reveal-up mt-8">
            <div className="absolute inset-0 bg-white/5 backdrop-blur-md border border-white/10 rounded-full transition-all duration-500 group-hover:bg-[#FF4D4D]/10 text-white"></div>
            <span className="relative z-10 font-['Space_Grotesk'] text-white/70 font-bold text-[10px] tracking-[0.4em] uppercase transition-colors duration-500 group-hover:text-white">Toca para Entrar</span>
            <div className="relative z-10 ml-8 flex items-center justify-center w-12 h-12 rounded-full border border-white/10 bg-white/5 transition-all duration-500 group-hover:bg-[#FF4D4D] group-hover:translate-x-1">
      ...
      <section id="experiencia" className="py-24 md:py-40 px-6 md:px-12 z-10 relative">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 bg-black/40 p-8 md:p-12 rounded-[40px] backdrop-blur-sm border border-white/5 floating-item">
          <div className="w-full md:w-1/2 overflow-hidden rounded-2xl aspect-[4/5]">
             <img src="https://images.pexels.com/photos/410648/pexels-photo-410648.jpeg" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-1000 scale-110 hover:scale-100" alt="Parrilla" />
          </div>
          <div className="w-full md:w-1/2 space-y-8">
            <h2 className="font-['Montserrat'] font-black text-4xl md:text-6xl uppercase tracking-tighter reveal-up">No es solo <br/> una cena</h2>
            <p className="font-['Cormorant_Garamond'] italic text-3xl md:text-4xl text-white/40 leading-tight reveal-up">"La tradición argentina, elevada en el corazón de España."</p>
            <p className="text-lg text-white/60 font-light leading-relaxed reveal-up">En Asado Central, respetamos la nobleza del producto. Cada corte es seleccionado a mano y sometido al dominio del fuego. No hay atajos, solo brasas.</p>
          </div>
        </div>
      </section>
      ...
      <section id="selección" className="py-24 md:py-40 px-6 md:px-12 z-10 relative text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {[
            { name: "Ojo de Bife", price: "34€", img: "https://images.pexels.com/photos/361184/pexels-photo-361184.jpeg" },
            { name: "Entraña Fina", price: "28€", img: "https://images.pexels.com/photos/7613568/pexels-photo-7613568.jpeg" },
            { name: "Costillar", price: "42€", img: "https://images.pexels.com/photos/361184/pexels-photo-361184.jpeg" }
          ].map((item, idx) => (
            <div key={idx} className="group bg-black/60 backdrop-blur-md rounded-3xl overflow-hidden border border-white/10 hover:border-[#FF4D4D]/30 transition-all duration-500">
      ...
      <section id="sedes" className="py-24 md:py-40 px-6 md:px-12 z-10 relative">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-12 bg-black/40 backdrop-blur-md p-12 rounded-[50px] border border-white/10">
          <div className="flex-1 space-y-12">
            <h2 className="font-['Montserrat'] font-black text-5xl md:text-8xl tracking-tighter reveal-up">MADRID</h2>
            <div className="space-y-4 reveal-up">
              <div className="flex items-center gap-4 text-white/50 text-sm"><MapPin size={16} className="text-[#FF4D4D]" /> Calle de Jorge Juan, 14</div>
              <p className="text-white/60 max-w-sm font-medium">Barrio de Salamanca, donde el lujo se rinde ante la brasa auténtica.</p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-12 md:pl-12">
            ...
            <div className="space-y-4 reveal-up">
              <div className="flex items-center gap-4 text-white/50 text-sm"><MapPin size={16} className="text-[#FF4D4D]" /> Passeig de Gràcia, 92</div>
      ...
      {/* FOOTER */}
      <footer className="py-20 px-6 border-t border-white/5 bg-black/80 backdrop-blur-md relative z-10 text-center">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          ...
          <p className="font-['Space_Grotesk'] text-[10px] uppercase tracking-[0.4em] text-white/20">© {new Date().getFullYear()} Asado Central. "El dominio del fuego."</p>
        </div>
      </footer>
    </div>
  );
